...
class Table:
    ...
    def create_delete_command(self, row, edit_window):
        def command():
            self.delete_row(row, edit_window)
            self.refresh()  # Refresh the table after deletion

        return command

    def delete_row(self, row, edit_window):
        ...
        if conn:
            ...
            delete_query = "DELETE FROM work_data WHERE `ID Number` = %s AND `Entry_Time` = %s"
            values = (row[1], row[6])  # ID Number and Entry_Time from the selected row
            ...
            # Close the edit window upon successful deletion
            edit_window.destroy()
    def edit_row(self, row, table_instance):
        ...
        edit_window.geometry('400x470')  # Change this if you want to adjust the size of the edit window
        ...
        update_button.pack()
        # Create a "Delete" button and pass edit_window as a parameter
        delete_button = Button(edit_window, text="Delete",
                               font=('Arial', 18, 'bold'),
                               command=self.create_delete_command(row, edit_window))
        delete_button.pack()
