...
def fetch_data_from_products(query, database):
    try:
        ...
        #print(f"TWO, Failed to fetch data from MySQL: {error}")
        print("huh")
        return []  # return an empty list in case of an error
    finally:
        if connection.is_connected():
...
def update_product_options(*args):
    ...
    product_options = fetch_data_from_proj1("SELECT product_name FROM all_products", 'productivitytrack')
...
if not test_connection():
    ...
    msg['CC'] = 'servicedesk@hme.com'
    msg['To'] = '#repairleadership@hme.com'  # Add the CC field to the email header
