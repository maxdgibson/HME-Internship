...
def insert_data():
    ...
    sql = "INSERT INTO workdata (`Date`, `Name`, `ID Number`, `Hours`, `Product`, `Operation`, `Quantity`, `Productivity`, `Hours_Gained`, `Comments`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
...
def update_product_options(*args):
    ...
    product_options = fetch_data_from_db("SELECT product_name FROM all_products", 'proj1')
...
# # Create the OptionMenu for operation selection
# operation_dropdown = tk.OptionMenu(window, operation_var, "Select Operation")
...
def update_operation_options(*args):
    ...
    operation_options = fetch_data_from_db(f"SELECT operation FROM {selected_product}", 'proj1')
...
window.geometry('980x340+800+200')
font_size = 20
...
# Set the initial options for the operation dropdown
...
# Define your 'window' and 'product_var' here, then create the OptionMenus
product_dropdown = tk.OptionMenu(window, product_var, *product_options)
operation_dropdown = tk.OptionMenu(window, operation_var, *operation_options)
# Set the initial options for the product dropdown
